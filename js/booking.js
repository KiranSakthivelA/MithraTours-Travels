...
function buildVehicleCard(vehicle, rawKm, tripType, pickup, drop, date, time, numDays) {
    ...
    const waMsg = encodeURIComponent(
        `*New Cab Booking — MTT Tours & Travels*\n\n` +
        `*From:* ${pickup}\n` +
        `*To:* ${drop}\n` +
        `*Date:* ${date}   *Time:* ${time}\n` +
        (tripType === 'Round Trip' ? `*No of Days:* ${nd}\n` : '') +
        `*Trip Type:* ${tripType}\n` +
    ...
    <div class="fm-vehicle-card" style="
        ...
        onmouseover="this.style.transform='translateY(-8px)';this.style.borderColor='#D97706';this.style.boxShadow='0 20px 45px rgba(217,119,6,0.28)'"
        ...
        <div style="position:relative; height:160px; overflow:hidden; background:${vehicle.color};">
            ...
            <span style="position:absolute; top:10px; right:12px; background:#D97706; color:#fff;
        ...
        <div style="padding:1.3rem 1.4rem; flex:1; display:flex; flex-direction:column; gap:0.55rem;">
            <div style="text-align:center; margin-bottom:0.5rem;">
                <div style="font-size:1.9rem; font-weight:900; color:#1A0A00; letter-spacing:-1px;">
            ...
            <div style="display:flex; gap:1rem; margin-top:0.3rem; font-size:0.77rem; color:#94a3b8;">
                <span><i class="fa-solid fa-user" style="color:#D97706;"></i> ${vehicle.seats}</span>
                <span><i class="fa-solid fa-suitcase" style="color:#D97706;"></i> ${vehicle.bags}</span>
            </div>
        </div>
        ...
        <div style="padding:0 1.4rem 1.4rem;">
            <button
               onclick="openBookingForm('${vehicle.label}','${pickup}','${drop}','${date}','${time}','${tripType}',${fare},${billableKm},'${battaLabel}', ${parseInt(vehicle.seats)}, ${parseInt(vehicle.bags)}, ${nd})"
               ...
               onmouseover="this.style.transform='scale(1.03)';this.style.boxShadow='0 8px 25px rgba(217,119,6,0.45)'"
...
window.openBookingForm = function(vehicleLabel, pickup, drop, date, time, tripType, fare, km, batta, maxPax, maxBags, numDaysArg) {
    ...
    <div style="max-width:540px; margin:0 auto; padding:2rem 1.5rem;">
        ...
        <div style="display:flex; flex-direction:column; gap:1rem;">
            ...
            <button
                onclick="sendWhatsAppBooking('${vehicleLabel}','${pickup}','${drop}','${date}','${time}','${tripType}',${fare},${km},'${batta}',${numDaysArg || 1})"
...
window.sendWhatsAppBooking = function(vehicleLabel, pickup, drop, date, time, tripType, fare, km, batta, numDaysParam) {
...
function initAutocomplete() {
    ...
    const GMAP_KEY = 'AIzaSyDjVfB0v9WlUtOzuifbE1LztOoWqrzyeJg';
...
async function handleQuickEstimate() {
    ...
    const baseDrop   = drop.split(',')[0].trim().toLowerCase();
    if (basePickup === baseDrop && basePickup !== '') {
...
async function handleMainEstimate() {
    ...
    const baseDropMain   = drop.split(',')[0].trim().toLowerCase();
    if (basePickupMain === baseDropMain && basePickupMain !== '') {
        showError(errorEl, 'Pick Up and Drop locations cannot be the same city.');
        return;
    }
    if (!date || !time) {
