...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Managers can now see actions too per rework request.
    if (role === 'manager') {
        const historyNav = document.getElementById('nav-history');
        // if (historyNav) historyNav.style.display = 'none'; // Optional: keep hidden if historical data is sensitive.
    }
    ...
    const checkInactivity = () => {
        ...
        if (inactivityTimer >= MAX_INACTIVITY) {
            window.location.href = "index.php?logout=true&timeout=true";
        }
    };
